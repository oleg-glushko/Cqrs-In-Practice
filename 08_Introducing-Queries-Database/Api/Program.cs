...
{
    var connectionString = builder.Configuration.GetConnectionString("CqrsInPracticeCommandsDb");
    options.UseSqlServer(connectionString, options =>
    {
        int retryCount = builder.Configuration.GetValue<int>("NumberOfDatabaseRetries");
        options.EnableRetryOnFailure(retryCount);
    });
    options.EnableSensitiveDataLogging();
});

builder.Services.AddScoped<DapperProvider>();
